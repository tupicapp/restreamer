...
type MismatchContext struct {
	MismatchIndex1 int
	MismatchIndex2 int
	BeforeFrames   []*shared.Frame // 5 frames before (from reference stream2)
	MismatchFrame1 *shared.Frame   // Mismatched frame from stream1
	MismatchFrame2 *shared.Frame   // Mismatched frame from stream2 (reference)
	AfterFrames    []*shared.Frame // 5 frames after (from reference stream2)
...
type MissingPacketContext struct {
	Index     int
	Frame     *shared.Frame
	RefFrame  *shared.Frame
	Before    []*shared.Frame
	After     []*shared.Frame
	RefBefore []*shared.Frame
	RefAfter  []*shared.Frame
	FrameHash string
}
...
type InputIDChange struct {
	...
	CurrentInput  string
	BeforeFrames  []*shared.Frame
	ChangeFrame   *shared.Frame
	AfterFrames   []*shared.Frame
}
...
func FrameHash(frame *shared.Frame) string {
	hasher := sha256.New()
...
func CheckFrameTiming(t *testing.T, frames []*shared.Frame, frameType string, expectedDuration, actualElapsed time.Duration, threshold float64) {
	if len(frames) == 0 {
		return
	}
...
func CheckSequenceIDContinuity(t *testing.T, frames []*shared.Frame, frameType string) {
	if len(frames) == 0 {
		return
	}
	...
	framesByInputID := make(map[string][]*shared.Frame)
	for _, frame := range frames {
		if frame == nil {
			continue
		}
...
func CheckSequenceIDContinuityForInput(t *testing.T, frames []*shared.Frame, frameType, inputID string) bool {
	if len(frames) == 0 {
		return false
	}
	...
	sequenceIDMap := make(map[int64]*shared.Frame)
...
func CheckH264FrameHealth(t *testing.T, frames []*shared.Frame) {
	if len(frames) == 0 {
		return
	}
...
func CompareFrameSequences(stream1, stream2 []*shared.Frame, frameType string) FrameSequenceComparisonResult {
...
func WindowMatchBenchmarkWithTiming(stream1, stream2 []*shared.Frame, frameType string, elapsedTime time.Duration, threshold float64) WindowMatchBenchmarkResult {
...
func WindowMatchBenchmark(stream1, stream2 []*shared.Frame, frameType string) WindowMatchBenchmarkResult {
...
func CollectMismatchContext(stream1, stream2 []*shared.Frame, mismatchIndex1, mismatchIndex2, windowIndex int) MismatchContext {
	ctx := MismatchContext{
		MismatchIndex1: mismatchIndex1,
		MismatchIndex2: mismatchIndex2,
		WindowIndex:    windowIndex,
		BeforeFrames:   make([]*shared.Frame, 0),
		AfterFrames:    make([]*shared.Frame, 0),
	}
...
// stream2 is used as the reference (source of truth)
func EqualPacketRateBenchmark(stream1, stream2 []*shared.Frame, frameType string) EqualPacketRateBenchmarkResult {
	...
	referenceStreamMap := make(map[int64]*shared.Frame)
...
func CollectFrameWindow(stream []*shared.Frame, idx, window int, before bool) []*shared.Frame {
	var res []*shared.Frame
	if before {
		start := idx - window
		if start < 0 {
			start = 0
...
func CollectNearestByPTS(stream []*shared.Frame, targetPTS time.Duration, window int, before bool) []*shared.Frame {
	if len(stream) == 0 {
		return nil
	}
	nearestIdx := 0
...
func findBaseTimestamp(stream []*shared.Frame) time.Time {
...
func formatPTSForLog(f *shared.Frame, baseTS time.Time) string {
	if f == nil {
		return "-"
	}
	if f.PTS != 0 {
...
func MeasureSwitchLatency(t *testing.T, videoFrames, audioFrames []*shared.Frame, switchEvents []SwitchEvent) {
...
func MeasureSwitchLatencyForFrames(t *testing.T, frames []*shared.Frame, switchEvents []SwitchEvent, frameType string) []SwitchLatencyResult {
...
func CheckInputIDChanges(t *testing.T, frames []*shared.Frame, frameType string) {
	if len(frames) == 0 {
		return
	}
...
func VerifyGOPIDCorrectness(t *testing.T, frames []*shared.Frame) {
	if len(frames) == 0 {
		return
	}
...
func CloneFramesWithDTSAsPTSSwitch(frames []*shared.Frame) []*shared.Frame {
...
func CloneFramesWithDTSAsPTS(frames []*shared.Frame) []*shared.Frame {
	out := make([]*shared.Frame, 0, len(frames))
	for _, f := range frames {
		if f == nil {
			continue
...
func isKeyFrame(frame *shared.Frame) bool {
	if frame == nil {
		return false
	}
